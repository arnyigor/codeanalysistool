...
- Учет контекстных файлов при анализе кода
  - Поддержка различных форматов файлов (Kotlin, Java, txt, MD, JSON, XML и др.)
  - Анализ целых директорий в качестве контекста
...
- `-c, --context` - Пути к файлам и/или директориям контекста через запятую (по умолчанию: пусто)
  - Поддерживаются различные типы файлов: код (.kt, .java), документация (.md, .txt), 
    конфигурации (.json, .yaml, .xml) и многие другие текстовые форматы
...
С директорией контекста:
```
analyze_code.bat path/to/MyClass.kt -c path/to/context/directory
```

С комбинацией файлов и директорий контекста:
```
analyze_code.bat path/to/MyClass.kt -c path/to/BaseClass.kt,path/to/context/directory
```

С очисткой кэша и логов:
```
analyze_code.bat path/to/MyClass.kt --clean-cache --clear-logs
```
