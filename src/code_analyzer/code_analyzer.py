...
class CodeAnalyzer:
    ...
    def analyze_path(self, path: str, recursive: bool = True,
        ...
        # Сначала подсчитываем общее количество файлов для анализа
        total_files = 0
        if path_obj.is_file():
            total_files = 1
        elif path_obj.is_dir():
            total_files = len(self.find_files(path_obj, recursive))

        logging.info("\n=== НАЧАЛО АНАЛИЗА ===")
        logging.info(f"Путь для анализа: {path}")
        logging.info(f"Всего файлов для анализа: {total_files}")
        logging.info(f"Рекурсивный анализ: {'Да' if recursive else 'Нет'}")
        logging.info(f"Директория для результатов: {output_dir or 'Не указана'}")
        logging.info("=" * 50)
        ...
        if path_obj.is_file():
            ...
            try:
                ...
                if result:
                    ...
                    logging.info(f"Файл успешно проанализирован: {path_obj}")
                else:
                    logging.warning(f"Не удалось проанализировать файл: {path_obj}")
            except Exception as e:
                logging.error(f"Ошибка при анализе файла {path_obj}: {str(e)}")

            logging.info("\n=== АНАЛИЗ ЗАВЕРШЕН ===")
            logging.info("Прогресс: 100% (1/1 файл)")
            logging.info("=" * 50)
            ...
            for file_path in self.find_files(path_obj, recursive):
                processed_files += 1
                ...
                try:
                    ...
                    if result:
                        ...
                        success_count += 1
                        logging.info(f"Файл успешно проанализирован: {file_path}")
                    else:
                        error_count += 1
                        logging.warning(f"Не удалось проанализировать файл: {file_path}")
                except Exception as e:
                    error_count += 1
                    logging.error(f"Ошибка при анализе файла {file_path}: {str(e)}")

            logging.info("\n=== АНАЛИЗ ЗАВЕРШЕН ===")
            logging.info(f"Обработано файлов: {processed_files}/{total_files}")
    ...
    def find_file_in_output_dir(self, name: str) -> Optional[str]:
        for root, _, files in os.walk(os.path.abspath("output")):
            if name in files:
    ...
    def analyze_file(self, file_path: str, context_files: List[str] = None) -> Optional[Dict]:
        ...
        if cached_result:
            logging.info(
                f"Результат загружен из кэша: {file_path}, проверяем наличие файла с документацией")

            # Извлекаем имя файла из cached_result
            file_name = os.path.basename(file_path)

            # Поиск файла в папке output
            found_file_path = self.find_file_in_output_dir(file_name)

            if found_file_path:
                logging.info(f"Файл с документацией найден: {found_file_path}")
                return cached_result
            else:
                logging.warning(f"Файл с документацией не найден: {file_path}")
        ...
        try:
            ...
            if context_files:
                ...
                if context:
                    logging.info(
                        f"Подготовлен контекст из {len(context)} файлов для анализа {file_path}")
                else:
                    logging.warning(
                        f"Контекстные файлы указаны, но не удалось прочитать ни один из них для {file_path}")
    ...
    def find_files(self, path: Path, recursive: bool) -> List[Path]:
        """
            ...
            try:
                ...
                if path_obj.exists() and path_obj.is_file():
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        logging.info(
                            f"Успешно загружен контекстный файл: {abs_path} (размер: {file_size} байт)")
                else:
                    failed_files += 1
                    logging.warning(
                        f"Контекстный файл не найден или не является файлом: {abs_path}")
            except Exception as e:
                failed_files += 1
                logging.error(f"Ошибка при чтении контекстного файла {file_path}: {str(e)}")

        logging.info(
            f"Загрузка контекста завершена: успешно загружено {successful_files} файлов, не удалось загрузить {failed_files} файлов")
        return context
    ...
    def _validate_documentation(self, documentation: str) -> str:
        ...
        if starts_with_backticks:
            ...
            if first_newline > 0:
                ...
                logging.info(
                    f"Удалены начальные тройные кавычки, новый размер: {len(documentation)}")
        ...
        if ends_with_backticks:
            ...
            if last_backticks_pos > 0:
                ...
                if last_newline > 0:
                    ...
                    logging.info(
                        f"Удалены конечные тройные кавычки, новый размер: {len(documentation)}")
        ...
        if "```" in documentation:
            ...
            while "```" in documentation:
                ...
                if end_pos == -1:
                    ...
                    documentation = documentation[:start_pos] + content_between + documentation[
                                                                                  end_pos + 3:]
                    logging.info(
                        f"Удален блок тройных кавычек, сохранено содержимое ({len(content_between)} символов)")

        # Проверяем наличие KDoc комментариев
        has_kdoc_start = "/**" in documentation
        has_kdoc_end = "*/" in documentation
        logging.info(
            f"Проверка KDoc маркеров: /** найден: {has_kdoc_start}, */ найден: {has_kdoc_end}")
        ...
        if not documentation.strip().startswith("/**"):
            ...
            if kdoc_start >= 0:
                ...
                logging.info(
                    f"Начало /** найдено на позиции {kdoc_start}, новый размер: {len(documentation)}")
            else:
                ...
                looks_like_code = any(keyword in documentation for keyword in
                                      ["class ", "interface ", "fun ", "val ", "var ", "package "])
                if looks_like_code:
                    logging.error(
                        f"Документация похожа на код, возможно модель сгенерировала код вместо документации. Примерно: '{documentation[:100]}...'")
                ...
                documentation = f"/**\n * {formatted_doc_body}\n */"
                logging.info(
                    f"Документация обернута в KDoc формат, новый размер: {len(documentation)}")
        ...
        if not documentation.strip().endswith("*/"):
            ...
            if kdoc_end >= 0:
                ...
                logging.info(
                    f"Конец */ найден на позиции {kdoc_end}, новый размер: {len(documentation)}")
            else:
        ...
        if removed_count > 0:
            documentation = '\n'.join(filtered_lines)
            logging.info(
                f"Удалено {removed_count} строк с '[Краткое описание]', новый размер: {len(documentation)}")

        # Финальная проверка
        if not (documentation.strip().startswith("/**") and documentation.strip().endswith("*/")):
            logging.error(
                f"После валидации документация не соответствует KDoc формату: {documentation[:50]}...{documentation[-50:]}")
            if not has_kdoc_start and not has_kdoc_end:
                ...
                if cleaned_text:
                    ...
                    logging.info(
                        f"Пытаемся восстановить документацию из текста: '{cleaned_text[:100]}...'")
                    # 1. Выполняем замену и сохраняем результат в переменную
        ...
        logging.info(
            f"Валидация документации завершена успешно, финальный размер: {len(documentation)}")
    ...
    def _extract_package_and_imports(self, file_content: str) -> tuple:
        ...
        while code_start_index < len(lines) and (
                not lines[code_start_index].strip() or lines[code_start_index].strip().startswith(
            "//")):
            code_start_index += 1
    ...
    def _save_result(self, file_path: str, result: Dict, output_dir: str) -> None:
        ...
        try:
            ...
            package_statement, imports, class_code = self._extract_package_and_imports(
                original_content)
            logging.debug(
                f"Извлечены секции - пакет: {bool(package_statement)}, импортов: {len(imports)}")
