...
2. Документация методов:
   ...
   - Указать используемые сервисы и утилиты"""
        ...
        prompt_template = f"""Вы - опытный разработчик, создающий документацию ИСКЛЮЧИТЕЛЬНО на РУССКОМ языке.
Ваша задача - добавить подробную документацию к коду.

{doc_rules}

[ТРЕБОВАНИЯ]
1. Документация ТОЛЬКО на РУССКОМ языке
2. Документировать класс и методы
3. НЕ добавлять @author, @version, @since
4. НЕ Добавлять комментарии после свойств
5. Документация в формате /** ... */первая строка класса или метода
6. Обязательно указывать все внешние зависимости
7. Описывать взаимодействие с другими компонентами
8. Вернуть подробную документацию к классу и методам и его взимосвязи
9. Вернуть СТРОГО ТОЛЬКО документацию для класса и методов, БЕЗ кода, БЕЗ импортов

[КОД]
{code}

[КОНЕЦ КОДА]
Пожалуйста, верните документацию с акцентом на связи и зависимости, БЕЗ КОДА."""
